...
def create_MyPyTable_for_auto_set(auto_dataset):
    ...
    auto_dataset_explored = MyPyTable(column_names=["pricesold","yearsold","Make","Year","BodyType","NumCylinders","DriveType"])

    for i in range(0, len(price_sold)):
        auto_dataset_explored.data.append([price_sold[i], year_sold[i], make[i], year[i], body_type[i], num_cylinders[i], drive_type[i]])
